...
class Triangle {
    ...
    static  float [] mProjMatrix=new float[16];//4*4投影矩阵
    static  float [] mVMatrix=new float[16];//摄像机位置朝向的参数矩阵

    private int mProgram;//自定义的渲染管线程序id
    private int muMVPMatrixHandle;//总的变换矩阵引用
    private int maPositionHandle;//顶点位置属性的应用
    private int maColorHandle;//顶点颜色属性的引用

    private static  float[] mMMatrix=new float[16];//具体物体的移动旋转矩阵，包括旋转，平移，缩放
    private FloatBuffer mVertexBuffer;//顶点坐标数据缓冲
    private FloatBuffer mColorBuffer;//顶点着色数据缓冲

    private int vCount=0;
    float xAngle=0;//绕x轴旋转的角度
    Triangle(MyTDView mv, Context context){
    ...
    */
    //从指定的glsl格式读取代码
    private void initShaderGLSL(MyTDView mv)
    {
        // 加载顶点着色器的脚本内容
        String mVertexShader = ShaderUtil.loadFromRawFile(context,
                R.raw.vertex);
        // 加载片元着色器的脚本内容
        String mFragmentShader = ShaderUtil.loadFromRawFile(context,
                R.raw.frag);
        //基于顶点着色器与片元着色器创建程序
        mProgram = ShaderUtil.createProgram(mVertexShader, mFragmentShader);
        //获取程序中顶点位置属性引用
    ...
    private static float[] getFianlMatrix(float[] spec)
    {
